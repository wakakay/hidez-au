...
class ModelCatalogOption extends Model {
	public function addOption($data) {
		...
		foreach ($data['option_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "option_description SET option_id = '" . (int)$option_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', classname = '" . $this->db->escape($value['classname']) . "', explainname = '" . $this->db->escape($value['explainname']) . "', explainhref = '" . $this->db->escape($value['explainhref']) . "'");
		}
		...
		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
				...
				foreach ($option_value['option_value_description'] as $language_id => $option_value_description) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value_description SET option_value_id = '" . (int)$option_value_id . "', language_id = '" . (int)$language_id . "', option_id = '" . (int)$option_id . "', name = '" . $this->db->escape($option_value_description['name']) . "', option_parent = '" . $this->db->escape($option_value_description['option_parent']) . "'");
				}
			}
		}
	}
	...
	public function getOptionDescriptions($option_id) {
		...
		foreach ($query->rows as $result) {
			$option_data[$result['language_id']] = array('name' => $result['name'], 'classname' => $result['classname'], 'explainname' => $result['explainname'], 'explainhref' => $result['explainhref']);
		}
	...
	public function getOptionValueDescriptions($option_id) {
		...
		foreach ($option_value_query->rows as $option_value) {
			...
			foreach ($option_value_description_query->rows as $option_value_description) {
				$option_value_description_data[$option_value_description['language_id']] = array('name' => $option_value_description['name'],'option_parent' => $option_value_description['option_parent']);
			}
