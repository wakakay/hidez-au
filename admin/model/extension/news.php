...
class ModelExtensionNews extends Model {
	public function addNews($data) {
		...
		foreach ($data['news'] as $key => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX ."news_description SET news_id = '" . (int)$news_id . "', language_id = '" . (int)$key . "', subtit = '" . $this->db->escape($value['subtit']) . "', newsIntroduction = '" . $this->db->escape($value['newsIntroduction']) . "', title = '" . $this->db->escape($value['title'])  . "',category = '" .  $this->db->escape($value['category']) . "', images = '" . $this->db->escape($value['images']) . "', description = '" . $this->db->escape($value['description']) . "'");
		}
	...
	public function editNews($id, $data) {
		...
		foreach ($data['news'] as $key => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX ."news_description SET news_id = '" . (int)$id . "', language_id = '" . (int)$key . "', subtit = '" . $this->db->escape($value['subtit']) . "', newsIntroduction = '" . $this->db->escape($value['newsIntroduction']) . "', title = '" . $this->db->escape($value['title']) . "', category = '" . $this->db->escape($value['category']). "', images = '" . $this->db->escape($value['images']) .   "', description = '" . $this->db->escape($value['description']) . "'");
		}
	...
	public function getNewsDescription($id) {
		...
		foreach ($query->rows as $result) {
			$news_description[$result['language_id']] = array(
				'title'       => $result['title'],
				'subtit'       => $result['subtit'],
				'newsIntroduction'  => $result['newsIntroduction'],
