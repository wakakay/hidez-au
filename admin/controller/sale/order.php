			...
			foreach ($products as $product) {
				...
				foreach ($options as $option) {
					if (isset($option['type']) && $option['type'] != 'file') {
						...
						$option_data[] = array(
							'name'  => $option['name'],
							'value' =>  isset($option['value']) ? utf8_substr($option['value'], 0, utf8_strrpos($option['value'], '.')) : '',
							'type'  => isset($option['type']) ? $option['type'] : '',
							'href'  => isset($option['order_option_id']) ? $this->url->link('sale/order/download', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'] . '&order_option_id=' . $option['order_option_id'], 'SSL') : '/'
						);						
					}
				}
