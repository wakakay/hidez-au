...
class ControllerCatalogProduct extends Controller {
	private $error = array(); 
  	...
  	public function insert() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_maxprice'])) {
				$url .= '&filter_maxprice=' . $this->request->get['filter_maxprice'];
			}
	...
	protected function getList() {				
		...
		if (isset($this->request->get['filter_maxprice'])) {
			$filter_maxprice = $this->request->get['filter_maxprice'];
		} else {
		...
		$this->data['column_maxprice'] = $this->language->get('column_maxprice');	
		...
		$this->data['sort_maxprice'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=p.maxprice' . $url, 'SSL');
		...
		$this->data['filter_maxprice'] 	= $filter_maxprice;
	...
	protected function getForm() {
		...
		$this->data['entry_maxprice'] = $this->language->get('entry_maxprice');
		$this->data['entry_us_price'] = $this->language->get('entry_us_price'); // 美国域名下的价格
        $this->data['entry_us_maxprice'] = $this->language->get('entry_us_maxprice');
    ...
    $this->data['tab_image'] = $this->language->get('tab_image');		
		...
		if (isset($this->request->post['price'])) {
      	...
      	$this->data['price'] = '599.00';
		}
		
		if (isset($this->request->post['maxprice'])) {
				$this->data['maxprice'] = $this->request->post['maxprice'];
		} elseif (!empty($product_info)) {
				$this->data['maxprice'] = $product_info['maxprice'];
		} else {
      	$this->data['maxprice'] = '';
		}
		...
		if (isset($this->request->post['z_id'])) {
			...
			if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
				...
				foreach ($product_option['product_option_value'] as $product_option_value) {
					$product_option_value_data[] = array(
						...
						'us_price'                => $product_option_value['us_price'],
                        'us_price_prefix'        => $product_option_value['us_price_prefix'],
							...
							foreach ($product_option['product_option_value'] as $product_option_value) {
								...
								if ($option_value_info) {
									$option_value_data[] = array(
										...
										'price_prefix'            => $product_option_value['price_prefix'],
										'us_price'                   => (float)$product_option_value['us_price'] ? $this->currency->format($product_option_value['us_price'], $this->config->get('config_currency')) : false,
                                        'us_price_prefix'            => $product_option_value['us_price_prefix']
									);
								}
							}
