...
class ModelCatalogCategory extends Model {
    public function getProductCategory($product_id) {
        $query = $this->db->query("SELECT parent_id FROM " . DB_PREFIX . "category WHERE category_id = (SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "')");
	...
	public function getCategories($parent_id = 0) {
        if ($parent_id != 0) {
            $query = $this->db->query("SELECT c.*, cd.*, p.price, p.maxprice FROM " . DB_PREFIX . "category c
                LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id)
                LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (c.category_id = c2s.category_id)
                LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (c.category_id = p2c.category_id)
                LEFT JOIN " . DB_PREFIX . "product p ON (p.product_id = p2c.product_id && p.status = 1)
