...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$this->data['text_colors_option'] = $this->language->get('text_colors_option');
			...
			foreach ($results as $result) {
				$this->data['images'][] = array(
				    'to_option_name' => $result['to_option_name'],
			...
			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				if($product_info['maxprice']>0 && $product_info['maxprice'] != $product_info['price']){
					if($product_info['maxprice']>$product_info['price']){
						$this->data['price'] = $this->currency->formatProduct($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax'))).'<strong class="scope"> - '.$this->currency->formatProduct($this->tax->calculate($product_info['maxprice'], $product_info['tax_class_id'], $this->config->get('config_tax')))."</strong>";
					}else{
						$this->data['price'] = '<strong class="scope">'.$this->currency->formatProduct($this->tax->calculate($product_info['maxprice'], $product_info['tax_class_id'], $this->config->get('config_tax'))).' - </strong>'.$this->currency->formatProduct($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')));
					}
				}else{
			...
			foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id']) as $option) {
				if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') {
					...
					foreach ($option['option_value'] as $option_value) {
						if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
							...
							$option_value_data[] = array(
								...
								'option_parent'           => $option_value['option_parent'],
								'image'                   => $this->model_tool_image->resize($option_value['image'], 800, 620),
								'price'                   => $price,
					...
					$this->data['options'][] = array(
						...
						'explainname'       => $option['explainname'],
						'explainhref'       => $option['explainhref'],
